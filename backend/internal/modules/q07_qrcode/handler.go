...
func (h *Handler) Create(c *gin.Context) {
	...
	if !CodeRegex.MatchString(code) {
		common.Fail(c, http.StatusBadRequest, "product_code must match XXXXX-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX (A-Z, 0-9)")
		return
	}
	var existing QRProduct
	if err := h.db.Where("product_code = ?", code).First(&existing).Error; err == nil {
		common.Fail(c, http.StatusConflict, "product_code already exists")
		return
	...
	row := QRProduct{ProductCode: code}
...
func (h *Handler) Delete(c *gin.Context) {
	...
	if err := h.db.Delete(&QRProduct{}, id).Error; err != nil {
		common.Fail(c, http.StatusInternalServerError, err.Error())
		return
	}
